...
contract Bridge is BridgeAdmin, Pausable {
    ...
    string public constant name = "Alyx Bridge";
    ...
    mapping (address=>mapping(address=>uint256)) public operatorTokenLimits;
    ...
    event TransferToken(address indexed caller, address indexed to, address indexed token, uint value);
    event TransferNative(address indexed caller, address indexed to, uint value);
    ...
    function depositNative(string memory _targetAddress, string memory chain, uint value, bool buyNative) public payable whenNotPaused{
        ...
        require(value >= minDeposit[chain][native], "INVALID_CROSS_TRANSFER_AMOUNT: less than minimum amount");
    ...
    function depositToken(address _token, uint value, string memory _targetAddress, string memory chain, bool buyNative) public payable whenNotPaused returns (bool){
        ...
        require(value >= minDeposit[chain][_token], "INVALID_CROSS_TRANSFER_AMOUNT: less than minimum amount");
    ...
    returns(bool)
    {
        require(value <= operatorTokenLimits[msg.sender][native], "exceed operator allowed token limits");
    ...
    returns (bool)
    {
        require(value <= operatorTokenLimits[msg.sender][_token], "exceed operator allowed token limits");
        require(taskHash == keccak256((abi.encodePacked(to,value,proof))),"task hash is wrong");
    ...
    whenPaused onlyOwner external
    {
        ...
        emit TransferNative(msg.sender, to, value);
    }
    ...
    whenPaused onlyOwner external
    {
        ...
        emit TransferToken(msg.sender, to, token, value);
    }
    ...
    function setOperatorTokenLimits(address operator, address token, uint256 _amount) 
    whenPaused onlyPauser external
    {
        operatorTokenLimits[operator][token] = _amount;
    }
